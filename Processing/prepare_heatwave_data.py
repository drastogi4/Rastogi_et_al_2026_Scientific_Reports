...
def load_one_file(fp: Path):
    ...
    keep_cols = [
        ...
        "tmaxCONUS","tmaxCAR","tmaxCENT","tmaxFL","tmaxMIDA","tmaxMIDW",
        "tmaxNE","tmaxNY","tmaxSE","tmaxTEN","tmaxSW","tmaxCAL","tmaxTEX","tmaxNW"
    ]
...
def build_combined_df(data_dir: Path):
    files = sorted(data_dir.glob("*_hw.csv"))
    if not files:
        raise FileNotFoundError(f"No *_hw.csv files found in {data_dir.resolve()}")

    dfs = [load_one_file(fp) for fp in files]
    df_all = pd.concat(dfs, ignore_index=True)

    # make sure numeric cols are numeric
    numeric_like = [c for c in df_all.columns if c.startswith(("parea","tmax","duration"))]
...
def reshape_long(df_all: pd.DataFrame):
    """Melt parea* and tmax* to long format and merge; also attach duration per event group."""
    id_vars = ["year","esm","sim","method","reference","period"]

    parea_cols = [c for c in df_all.columns if c.startswith("parea")]
    tmax_cols  = [c for c in df_all.columns if c.startswith("tmax")]
    ...
    df_tmax = df_all.melt(
        id_vars=id_vars,
        value_vars=tmax_cols,
        var_name="region_var",
        value_name="tmax"
    )
    df_tmax["region"] = df_tmax["region_var"].str.replace("^tmax", "", regex=True)

    # merge parea & tmax
    df_long = pd.merge(
        df_parea.drop(columns=["region_var"]),
        df_tmax.drop(columns=["region_var"]),
        on=id_vars + ["region"],
        how="outer"
    )
...
def compute_future_minus_historical(df_annual: pd.DataFrame):
    ...
    for v in ["parea_mean","parea_max","parea_sum","tmax_mean","tmax_max","duration_mean","n_events"]:
